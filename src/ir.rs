...
pub enum InstKind {
    ...
    Field { base: ValueId, record: usize, field: usize },
...
impl LoweredProgram {
    pub fn lower(source: ast::Program) -> Result<Self, String> {
        ...
        for f in &source.props {
            ...
            properties.push(Property {
                function: id,
                name: f.name.clone(),
                params: f.params.iter().map(|(name, ty)| Ok((name.clone(), crate::check::resolve_type(&source, ty)?))).collect::<Result<_, String>>()?,
            });
        }
        let main = source.main.as_ref().map(|body| {
            let decl = FnDecl {
                name: "main".into(),
                params: Vec::new(),
                inouts: Vec::new(),
    ...
    pub fn validate(&self) -> Result<(), String> {
        for f in self.functions.iter().chain(self.main.iter()) {
            if f.entry as usize >= f.blocks.len() { return Err(format!("IR `{}` has invalid entry", f.name)); }
            if f.params.len() != f.inouts.len() || f.locals.iter().any(|local| local.name.is_empty()) {
                return Err(format!("IR `{}` has invalid local metadata", f.name));
            }
            for (&param, &inout) in f.params.iter().zip(&f.inouts) {
                let local = f.locals.get(param as usize).ok_or_else(|| format!("IR `{}` has invalid parameter", f.name))?;
                if inout && !local.mutable { return Err(format!("IR `{}` has immutable inout parameter", f.name)); }
            }
            for (bi, block) in f.blocks.iter().enumerate() {
                for inst in &block.instructions {
                    if let Some(v) = inst.result {
                        if f.values.get(v as usize) != Some(&inst.ty) {
                            return Err(format!("IR `{}` block {} has mistyped value %{}", f.name, bi, v));
                        }
                    }
                    for v in operands(&inst.kind) {
                        if v as usize >= f.values.len() {
                            return Err(format!("IR `{}` block {} uses invalid value %{}", f.name, bi, v));
                        }
                    }
                    match &inst.kind {
                        InstKind::Load(local) => {
                        ...
                        InstKind::Call { callee: Callee::Function(id), args, inout } => {
                            let callee = self.functions.get(*id as usize).ok_or_else(|| format!("IR `{}` calls invalid function", f.name))?;
                            if args.len() != callee.params.len() || inout.len() != args.len() { return Err(format!("IR `{}` call arity mismatch", f.name)); }
                ...
                let target = |b: BlockId| -> Result<(), String> {
                    if (b as usize) < f.blocks.len() { Ok(()) } else { Err(format!("IR `{}` block {} jumps to invalid block {}", f.name, bi, b)) }
                };
                match block.terminator {
                    Terminator::Jump(b) => target(b)?,
                    Terminator::Branch { condition, then_block, else_block } => {
                        if f.values.get(condition as usize) != Some(&Type::Bool) {
                            return Err(format!("IR `{}` block {} branches on non-bool", f.name, bi));
                        }
                        target(then_block)?; target(else_block)?;
                    }
                    Terminator::Return(v) => {
                        let got = f.values.get(v as usize).ok_or_else(|| format!("IR `{}` returns invalid value", f.name))?;
                        if !compatible(&f.ret, got) { return Err(format!("IR `{}` returns {:?}, expected {:?}", f.name, got, f.ret)); }
                    }
...
mod tests {
    ...
    fn lowering_resolves_calls_fields_and_short_circuit_control() {
        ...
        assert!(main.blocks.iter().any(|b| matches!(b.terminator, Terminator::Branch { .. })));
        assert!(main.blocks.iter().flat_map(|b| &b.instructions).any(|i| {
            matches!(i.kind, InstKind::Call { callee: Callee::Function(0), .. })
        }));
        assert!(main.blocks.iter().flat_map(|b| &b.instructions).any(|i| {
            matches!(i.kind, InstKind::Field { record: 0, field: 0, .. })
        }));
    }
...
impl<'a> Builder<'a> {
    fn new(p: &'a ast::Program, types: &'a [Option<Type>], functions: &'a HashMap<String, FunctionId>, f: &FnDecl) -> Result<Self, String> {
        ...
        for (i, (name, ty)) in f.params.iter().enumerate() {
            let id = b.add_local(name, crate::check::resolve_type(p, ty)?, f.inouts.get(i).copied().unwrap_or(false));
            b.params.push(id);
        }
        Ok(b)
    }
    ...
    fn finish(mut self, body: &[StmtId]) -> Result<Function, String> {
        ...
        if matches!(self.blocks[self.current as usize].terminator, Terminator::Unreachable) {
            let value = match last {
                Some(v) if compatible(&self.ret, &self.values[v as usize]) => v,
                _ if self.ret == Type::Unit => self.constant(Constant::Unit, Type::Unit),
                _ => return Err(format!("lowering `{}` found no fallthrough value", self.name)),
            };
    ...
    fn add_local(&mut self, name: &str, ty: Type, mutable: bool) -> LocalId {
        ...
        self.locals.push(Local { name: name.into(), ty, mutable });
        self.scopes.last_mut().unwrap().insert(name.into(), id);
        id
    }
    fn temp_local(&mut self, ty: Type) -> LocalId {
        let id = self.locals.len() as LocalId;
        self.locals.push(Local { name: format!("$tmp{}", id), ty, mutable: true });
        id
    }
    fn lookup(&self, name: &str) -> Result<LocalId, String> {
        self.scopes.iter().rev().find_map(|s| s.get(name).copied()).ok_or_else(|| format!("lowering: unknown local `{}`", name))
    }
    fn new_block(&mut self) -> BlockId {
        let id = self.blocks.len() as BlockId;
        self.blocks.push(Block { instructions: Vec::new(), terminator: Terminator::Unreachable });
        id
    }
    fn switch(&mut self, id: BlockId) { self.current = id; }
    fn terminate(&mut self, term: Terminator) { self.blocks[self.current as usize].terminator = term; }
    fn emit(&mut self, kind: InstKind, ty: Type) -> ValueId {
        let id = self.values.len() as ValueId;
        self.values.push(ty.clone());
        self.blocks[self.current as usize].instructions.push(Inst { result: Some(id), ty, kind });
        id
    }
    fn effect(&mut self, kind: InstKind) {
        self.blocks[self.current as usize].instructions.push(Inst { result: None, ty: Type::Unit, kind });
    }
    ...
    fn block(&mut self, stmts: &[StmtId]) -> Result<Option<ValueId>, String> {
        ...
        let mut last = None;
        for &s in stmts {
            if !matches!(self.blocks[self.current as usize].terminator, Terminator::Unreachable) { break; }
            last = self.stmt(s)?;
        }
        self.scopes.pop();
        Ok(last)
    ...
    fn stmt(&mut self, sid: StmtId) -> Result<Option<ValueId>, String> {
        match self.p.stmt(sid) {
            ast::Stmt::Let(name, e) | ast::Stmt::Var(name, e) => {
                ...
                let local = self.add_local(name, self.expr_type(*e)?, matches!(self.p.stmt(sid), ast::Stmt::Var(..)));
                self.store(local, value); Ok(None)
            }
            ast::Stmt::Assign(target, e) => {
                ...
                match self.p.expr(*target) {
                    ...
                    ast::Expr::Field(..) => {
                        ...
                        let root = loop { match self.p.expr(cur) {
                            ast::Expr::Field(base, name) => { names.push(name.as_str()); cur = *base; }
                        ...
                        for name in names {
                            let Type::Rec(ti) = ty else { return Err("lowering: field on non-record".into()) };
                            let fi = self.p.types[ti].fields.iter().position(|(n, _)| n == name).ok_or_else(|| format!("unknown field `{}`", name))?;
                            ty = crate::check::resolve_type(self.p, &self.p.types[ti].fields[fi].1)?;
                            path.push(fi);
                        }
            ...
            ast::Stmt::If(c, yes, no) => {
                ...
                let tb = self.new_block(); let fb = self.new_block(); let merge = self.new_block();
                self.terminate(Terminator::Branch { condition, then_block: tb, else_block: fb });
                self.switch(tb); self.block(yes)?;
                if matches!(self.blocks[self.current as usize].terminator, Terminator::Unreachable) { self.terminate(Terminator::Jump(merge)); }
                self.switch(fb); self.block(no)?;
                if matches!(self.blocks[self.current as usize].terminator, Terminator::Unreachable) { self.terminate(Terminator::Jump(merge)); }
                self.switch(merge); Ok(None)
            }
            ast::Stmt::While(c, body) => {
                let head = self.new_block(); let loop_body = self.new_block(); let exit = self.new_block();
                self.terminate(Terminator::Jump(head)); self.switch(head);
                let condition = self.expr(*c)?;
                self.terminate(Terminator::Branch { condition, then_block: loop_body, else_block: exit });
                self.switch(loop_body); self.block(body)?;
                if matches!(self.blocks[self.current as usize].terminator, Terminator::Unreachable) { self.terminate(Terminator::Jump(head)); }
                self.switch(exit); Ok(None)
            }
            ast::Stmt::For(name, lo, hi, body) => {
                let lo = self.expr(*lo)?; let hi = self.expr(*hi)?;
                self.scopes.push(HashMap::new());
                let index = self.add_local(name, Type::I64, false); self.store(index, lo);
                let head = self.new_block(); let loop_body = self.new_block(); let exit = self.new_block();
                self.terminate(Terminator::Jump(head)); self.switch(head);
                let iv = self.load(index); let cond = self.emit(InstKind::Binary { op: BinaryOp::Lt, lhs: iv, rhs: hi }, Type::Bool);
                self.terminate(Terminator::Branch { condition: cond, then_block: loop_body, else_block: exit });
                self.switch(loop_body); self.block(body)?;
                if matches!(self.blocks[self.current as usize].terminator, Terminator::Unreachable) {
                    let cur = self.load(index); let one = self.constant(Constant::I64(1), Type::I64);
                    let next = self.emit(InstKind::Binary { op: BinaryOp::Add, lhs: cur, rhs: one }, Type::I64);
                    self.store(index, next); self.terminate(Terminator::Jump(head));
                }
                self.scopes.pop(); self.switch(exit); Ok(None)
            }
            ast::Stmt::Return(e) => {
                let value = match e { Some(e) => self.expr(*e)?, None => self.constant(Constant::Unit, Type::Unit) };
                self.terminate(Terminator::Return(value)); Ok(Some(value))
            }
    ...
    fn expr(&mut self, eid: ExprId) -> Result<ValueId, String> {
        ...
        match self.p.expr(eid) {
            ...
            ast::Expr::Ident(name) => { let l = self.lookup(name)?; Ok(self.load(l)) }
            ast::Expr::Un(op, e) => {
                let value = self.expr(*e)?;
                let op = match op.as_str() { "-" => UnaryOp::Neg, "not" => UnaryOp::Not, _ => return Err(format!("lowering: unknown unary `{}`", op)) };
                Ok(self.emit(InstKind::Unary { op, value }, ty))
            }
            ast::Expr::Bin(op, l, r) if op == "and" || op == "or" => self.short_circuit(op == "and", *l, *r),
            ast::Expr::Bin(op, l, r) => {
                let lhs = self.expr(*l)?; let rhs = self.expr(*r)?;
                if let Some(name) = self.p.infix_ops.get(op) {
                    let callee = Callee::Function(*self.functions.get(name).ok_or_else(|| format!("unknown operator fn `{}`", name))?);
                    Ok(self.emit(InstKind::Call { callee, args: vec![lhs, rhs], inout: vec![None, None] }, ty))
                } else {
                    let op = match op.as_str() { "+"=>BinaryOp::Add,"-"=>BinaryOp::Sub,"*"=>BinaryOp::Mul,"/"=>BinaryOp::Div,"%"=>BinaryOp::Rem,"=="=>BinaryOp::Eq,"!="=>BinaryOp::Ne,"<"=>BinaryOp::Lt,"<="=>BinaryOp::Le,">"=>BinaryOp::Gt,">="=>BinaryOp::Ge,"~="|"≈"=>BinaryOp::ApproxEq,_=>return Err(format!("lowering: unknown binary `{}`",op)) };
                    Ok(self.emit(InstKind::Binary { op, lhs, rhs }, ty))
                }
            }
            ast::Expr::Circum(open, e) => {
                let value = self.expr(*e)?; let name = &self.p.circum_ops[open].1;
                let callee = Callee::Function(*self.functions.get(name).ok_or_else(|| format!("unknown operator fn `{}`", name))?);
                Ok(self.emit(InstKind::Call { callee, args: vec![value], inout: vec![None] }, ty))
            }
            ast::Expr::Field(base, name) => {
                let base_ty = self.expr_type(*base)?; let Type::Rec(record) = base_ty else { return Err("lowering: field on non-record".into()) };
                let field = self.p.types[record].fields.iter().position(|(n, _)| n == name).ok_or_else(|| format!("unknown field `{}`", name))?;
                let base = self.expr(*base)?; Ok(self.emit(InstKind::Field { base, record, field }, ty))
            }
            ast::Expr::Index(base, index) => { let base=self.expr(*base)?; let index=self.expr(*index)?; Ok(self.emit(InstKind::Index { base,index },ty)) }
            ast::Expr::Array(items) => { let values=items.iter().map(|e| self.expr(*e)).collect::<Result<Vec<_>,_>>()?; Ok(self.emit(InstKind::Array(values),ty)) }
            ast::Expr::Record(name, fields) => {
                let record=self.p.types.iter().position(|t|t.name==*name).ok_or_else(||format!("unknown record `{}`",name))?;
                let mut ordered=vec![None;self.p.types[record].fields.len()];
                for (pos,(name,e)) in fields.iter().enumerate(){ let field=name.as_ref().map(|n|self.p.types[record].fields.iter().position(|(f,_)|f==n).unwrap()).unwrap_or(pos); ordered[field]=Some(self.expr(*e)?); }
                Ok(self.emit(InstKind::Record { record, fields: ordered.into_iter().map(Option::unwrap).collect() },ty))
            }
            ast::Expr::EnumVal(en,var) => { let (enumeration,tag)=self.p.enum_tag(en,var).ok_or_else(||format!("unknown enum `{}.{}`",en,var))?; Ok(self.emit(InstKind::Enum { enumeration,tag },ty)) }
    ...
    fn short_circuit(&mut self, and: bool, lhs: ExprId, rhs: ExprId) -> Result<ValueId,String> {
        let left=self.expr(lhs)?; let slot=self.temp_local(Type::Bool); self.store(slot,left);
        let right_block=self.new_block(); let merge=self.new_block();
        self.terminate(if and { Terminator::Branch{condition:left,then_block:right_block,else_block:merge} } else { Terminator::Branch{condition:left,then_block:merge,else_block:right_block} });
        self.switch(right_block); let right=self.expr(rhs)?; self.store(slot,right); self.terminate(Terminator::Jump(merge));
        self.switch(merge); Ok(self.load(slot))
    }
    ...
    fn sum(&mut self,var:&str,lo:ExprId,hi:ExprId,body:ExprId,ty:Type)->Result<ValueId,String>{
        ...
        let cur=self.load(index);let one=self.constant(Constant::I64(1),Type::I64);let inc=self.emit(InstKind::Binary{op:BinaryOp::Add,lhs:cur,rhs:one},Type::I64);self.store(index,inc);self.terminate(Terminator::Jump(head));
        self.scopes.pop();self.switch(exit);Ok(self.load(acc))
    }
}
